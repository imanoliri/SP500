...
def generate_growth_infos(df: pd.DataFrame, *, value_column: str=None, growth_column: str=None, range_start: int = 5, range_end: int = 41, range_step = 5) -> pd.DataFrame:
    ...
    df_growth = pd.DataFrame(index=growths.index,columns=pd.MultiIndex.from_product([(comp_growth_column,acc_growth_column),growth_spans]))
...
def generate_value_growth_infos(df: pd.DataFrame, *, value_column: str, range_start: int = 5, range_end: int = 41, range_step = 5) -> pd.DataFrame:
    growth_spans = list(range(range_start, range_end, range_step))
    growth_column ='growth'    

    values = df.loc[:,value_column]
    df_growth = pd.DataFrame(index=values.index,columns=pd.MultiIndex.from_product([[growth_column],growth_spans]))
