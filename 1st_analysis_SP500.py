...
df_growths.columns = pd.MultiIndex.from_tuples(
    [("sp500", *c) for c in df_growths.columns]
)
...
df_inflations.columns = pd.MultiIndex.from_tuples(
    [("prices", *c) for c in df_inflations.columns]
)
# %%
...
sp500_growth_vs_inflation_series = (
    (selected_strategy, "bar", "tab:blue", {}),
    (selected_inflation, "bar", "tab:red", {}),
)
sp500_retirements_vs_inflation_series = [
    (
        (sr, "bar", "tab:green", {}),
        (selected_inflation, "bar", "tab:red", {}),
    )
    for sr in selected_retirements
]

combined_data = (
    (df, df_all, *[df_all] * len(retirement_draws)),
    (
        sp500_value_series,
        sp500_growth_vs_inflation_series,
        *sp500_retirements_vs_inflation_series,
...
combined_plot(
    *combined_data,
    title="40-year_DCA_investment_&_retirements_vs_inflation",
    path=results_strategy_validation_dir,
    figsize=(20, 10 + 2.5 * len(retirement_draws)),
)
